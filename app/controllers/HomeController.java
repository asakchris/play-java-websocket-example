...
public class HomeController extends Controller {
    ...
    @Inject
    public HomeController(@Named("userParentActor") ActorRef userParentActor, WebJarsUtil webJarsUtil, ActorSystem actorSystem, Materializer materializer) {
    ...
    public WebSocket connect() {
        return WebSocket.Text.accept(request -> ActorFlow.actorRef(out -> EchoActor.props(out), actorSystem, materializer));
    }

    public WebSocket jsonConnect() {
        return WebSocket.json(Request.class).accept(request -> {
            logger.info("Received WebSocket JSON request: {}", request);
            return ActorFlow.actorRef(EchoActor::props, actorSystem, materializer);
        });
        /*return WebSocket.json(Request.class).acceptOrResult(request -> {
            logger.info("Received WebSocket JSON request: {}", request);
            return CompletableFuture
                    .completedFuture(F.Either.Right(ActorFlow.actorRef(out -> EchoActor.props(out), actorSystem, materializer)));
        });*/
    }
